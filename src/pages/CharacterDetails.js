...
function CharacterDetails() {
  ...
  const data = detail.filter((item) => item.id === id);
          ...
          <div className="grid grid-cols-1 gap-6 lg:grid-cols-12">
            ...
            <div className="col-span-1 lg:col-span-9">
              <div className="text-center lg:text-left">
                ...
                <p className="mt-2 font-semibold bg-pink-500 rounded p-1 text-white inline-block">
                  {data[0].role}
                </p>

                <h2 className="font-bold mt-2">Bio</h2>
                <p className="mt-2 text-zinc-500">{data[0].bio}</p>
                <h2 className="font-bold mt-2">Story</h2>
                <p className="mt-2 text-zinc-500">{data[0].story}</p>
              </div>
            </div>
          </div>
        </div>
